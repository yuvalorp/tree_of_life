...
from consts import CACHE_DIR, DATA_TABLE_VERSION, PERSON_INFO_CACHE_DIR, SOUP_CACHE_DIR
from src.html_utils import extract_data_from_table, get_full_wiki_url, get_info_table, get_soup
...
class _SoupBackCache(BaseCache):
    def __init__(self):
        super().__init__()
...
class SoupCache(BaseCache):
    def __init__(self):
        super().__init__()
        self._cache_dir = SOUP_CACHE_DIR
        self._soup_manager = _SoupBackCache()

    def get(self, resource_name: str, read_cache: bool, write_cache: bool):
        return self._soup_manager.get_or_load_resource(
            resource_name, read_cache, write_cache
        )

    def post_process(self, result):
        infobox = get_info_table(result)
        if infobox is None:
            return {}
        else:
...
class TableDataCache(BaseCache):
    ...
    def get(self, resource_name: str, read_cache: bool, write_cache: bool):
        soup = self._soup_manager.get_or_load_resource(resource_name, read_cache, write_cache=False)
